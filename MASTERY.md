...
| Socket | L2 | 2026-09-15 Echo 实验已跑通；2026-09-18 在逐步教学下完成多进程文本文件传输并留下源码证据。能解释两个 fd、ACK、TCP 字节流与分块收发；2026-09-19 已学习 select 概念，不据此升级。下一证据：独立写出 select 多客户端 Echo Server + 既有文件传输闭卷复现，并完成二进制/异常路径验证后再评估 L3 |
...
TCP 基础已覆盖握手/挥手、缓冲区、Nagle 和 listen，正式进入 select/bitmap/fd_set 学习。今天记录为概念学习与服务端流程讨论，尚无独立 select 实现或运行证据，Socket 仍保持 L2。
